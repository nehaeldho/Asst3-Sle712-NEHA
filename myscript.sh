
                                                                                                                        



#!/bin/bash
for i in {1..10} ; do

#to make blast index of cds sequence
formatdb -p F  -o T -i Escherichia_coli_str_k_12_substr_mg1655.ASM584v2.cds.all.fa

# to introduce mismatches in the sequence
msbar -sequence 8.fa -count 275 -point 4 -block 0 -codon 0 -outseq mysample_mutated.fa

# to run the blast
blast2 -p blastn -e 0.001 -m 8 -d Escherichia_coli_str_k_12_substr_mg1655.ASM584v2.cds.all.fa -i mysample_mutated.fa 

done
